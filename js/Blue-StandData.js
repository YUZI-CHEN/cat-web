...
// 1. 綁定藍線標籤點擊事件
document.querySelectorAll(".blue-tag").forEach(tag => {
  tag.addEventListener("click", blueTagHandler); // 綁定特定處理器
});

// 2. 綁定藍線地圖點擊事件
document.querySelectorAll(".blue-point").forEach(point => {
  point.addEventListener("click", bluePointHandler); // 綁定特定處理器
});
...
// 4. 定義事件處理器
function blueTagHandler(e) {
  const selectedTag = e.target.dataset.tag;
  const filteredInfos = blueLineData.filter(item => item.tags.includes(selectedTag));
  displayBlueInfos(filteredInfos);
}

function bluePointHandler(e) {
  const id = e.target.dataset.id;
  const infos = blueLineData.filter(item => item.id === id);
  displayBlueInfos(infos);
}