...
const redLineData = [
  ...
  function createPoints() {
    ...
    redLineData.forEach(location => {
      const point = document.createElement("div");
      point.className = "point";
      point.style.top = `${location.coords.y}px`;
      point.style.left = `${location.coords.x}px`;
      point.setAttribute("data-id", location.id);
      point.onclick = (event) => showInfo(event, location);
  ...
  function showInfo(event, location) {
    ...
    infoContent.innerHTML = `
      ...
      <p><strong>車站:</strong> ${location.station}</p>
      <p><strong>營業時間:</strong> ${location.time}</p>
      <p><strong>地址:</strong> ${location.address}</p>
      <p><strong>電話:</strong> ${location.tel}</p>
      ${
        location.web
          ? `<p><strong>網站:</strong> <a href="${location.web}" target="_blank">點擊查看</a></p>`
          : ""
      }
      <p><strong>地圖:</strong> <a href="${location.map}" target="_blank">點擊查看</a></p>
      <p><strong>其他:</strong> ${location.other}</p>
      <p><strong>標籤:</strong> ${location.tags.join(", ")}</p>
    `;
